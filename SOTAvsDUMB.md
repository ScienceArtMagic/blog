...
Using PE as an example: even if papers like NoPos/NoPE had failed miserably, I would have found them useful. One of the few papers on the subject even reported that non-casual (particularly encoder) models struggled without PE. That is, again, intuitive at least to some degree, but also useful information. In this case, it also validates the benefit of the causal mask in providing its own, unique (and potentially - in some cases empirically - superior) positional information.
